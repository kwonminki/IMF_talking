...
from IMF_talking.models.vit import ImplicitMotionAlignment
from IMF_talking.models.resblocks import FeatResBlock,UpConvResBlock,DownConvResBlock
from IMF_talking.models.lia_resblocks import StyledConv,EqualConv2d,EqualLinear,ResBlock # these are correct https://github.com/hologerry/IMF/issues/4  "You can refer to this repo https://github.com/wyhsirius/LIA/ for StyleGAN2 related code, such as Encoder, Decoder."
from IMF_talking.utils.tensor_utils import normalize
...
class IMFModel(nn.Module):
    def __init__(self,use_resnet_feature=False,use_mlgffn=False,use_skip=False,use_enhanced_generator=False, latent_dim=32, base_channels=64, num_layers=4, noise_level=0.1, style_mix_prob=0.5):
        ...
        self.device = torch.device('cpu')  # 초기화 시 기본값은 'cpu'
# ok
...
# ok
    ...
    def to(self, device):
        self.device = device  # 현재 디바이스를 업데이트
        super().to(device)  # nn.Module의 기본 to() 호출
        return self
